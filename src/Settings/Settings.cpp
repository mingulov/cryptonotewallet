...
const char DEFAULT_WALLET_FILE_NAME[] = "hervacoinwallet.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "52.166.108.254:33710";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("hervacoinwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("hervacoinwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("hervacoinwallet.desktop");
  ...
  res = autorunSettings.contains("HervaCoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("HervaCoinWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.hervacoin.hervacoin scientistwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=HervaCoin Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("HervaCoinWallet", appPath);
    } else {
      autorunSettings.remove("HervaCoinWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\hervacoin", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:turtlecoin");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\hervacoin\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\hervacoin\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("hervacoinwallet.cfg"));
