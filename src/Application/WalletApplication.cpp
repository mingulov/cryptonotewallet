...
const char BYTECOIN_URI_SCHEME_NAME[] = "hervacoin";
...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("hervacoinwallet");
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("hervacoinwallet.lock"));
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have bytecoind with non-default RPC port, you should terminate it and relaunch HervacoinWallet\n"
      "or\n"
      "Set the Local deamon required port in HervacoinWallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
